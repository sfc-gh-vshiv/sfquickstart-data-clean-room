...
2022-07-11          B. Klein                            Renamed _demo_ to _wmt_  and added template_type to
...
/////
// SFSENORTHAMERICA-VINODSHIV_GCPCENTRAL TEMPLATES
/////

// if you want to edit the templates, run this then re-insert them
// delete from dcr_wmt_provider_db.templates.dcr_templates;

//Change SFSENORTHAMERICA-VINODSHIV with consumer account name and adjust query template as per requirement

insert into dcr_wmt_provider_db.templates.dcr_templates (party_account,template_name, template, dimensions, template_type)
values ('SFSENORTHAMERICA-VINODSHIV','customer_overlap',
$$
select
    {% if dimensions %}
...
insert into dcr_wmt_provider_db.templates.dcr_templates (party_account,template_name, template, dimensions, template_type)
values ('SFSENORTHAMERICA-VINODSHIV','customer_overlap_multiparty',
$$
select
    identifier({{ dimensions[0] }})
    {% for dim in dimensions[1:] %}
...
insert into dcr_wmt_provider_db.templates.dcr_templates (party_account,template_name, template, dimensions, template_type)
values ('SFSENORTHAMERICA-VINODSHIV','customer_overlap_waterfall',
$$
select
    identifier({{ dimensions[0] }})
    {% for dim in dimensions[1:] %}
...
insert into dcr_wmt_provider_db.templates.dcr_templates (party_account,template_name, template, dimensions, template_type)
values ('SFSENORTHAMERICA-VINODSHIV','campaign_conversion',
$$
...
insert into dcr_wmt_provider_db.templates.dcr_templates (party_account,template_name, template, dimensions, template_type)
values ('SFSENORTHAMERICA-VINODSHIV','customer_overlap_multiparty_subscribers',
$$
select
    identifier({{ dimensions[0] }})
    {% for dim in dimensions[1:] %}
...
// see templates
// select * from dcr_wmt_provider_db.templates.dcr_templates;
...
select dcr_wmt_provider_db.templates.get_sql_jinja(
  (select template from dcr_wmt_provider_db.templates.dcr_templates where template_name = 'campaign_conversion'),
   object_construct(
