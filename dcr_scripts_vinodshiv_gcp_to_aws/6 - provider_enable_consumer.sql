...
2022-07-11          B. Klein                            Renamed _demo_ to _wmt_ to support upgrades.
...
/////
// REQUEST HANDLING FROM SFSENORTHAMERICA_VINODSHIV
/////

// create a new mounted request database, stream, and task for each consumer

////////
/// DEMO RESET SFSENORTHAMERICA_VINODSHIV_GCPCENTRAL SIDE
...
create or replace database dcr_wmt_SFSENORTHAMERICA_VINODSHIV from share SFSENORTHAMERICA_VINODSHIV.dcr_wmt_requests_SFSENORTHAMERICA_VINODSHIV_GCPCENTRAL;
create or replace stream dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV on table dcr_wmt_SFSENORTHAMERICA_VINODSHIV.SFSENORTHAMERICA_VINODSHIV_GCPCENTRAL_schema.requests append_only = true show_initial_rows = true
comment='{"origin":"sf_ps_wls","name":"dcr","version":{"major":5, "minor":5},"attributes":{"component":"dcr",“role”:“provider”}}';
delete from dcr_wmt_provider_db.admin.request_log;

// see request status and streams

// see requests
select * from dcr_wmt_SFSENORTHAMERICA_VINODSHIV.SFSENORTHAMERICA_VINODSHIV_GCPCENTRAL_schema.requests;
// see request stream
select * from dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV;
// see if the stream has anything in it
select SYSTEM$STREAM_HAS_DATA('dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV');
// see processed request log
select * from dcr_wmt_provider_db.admin.request_log;
...
create or replace procedure dcr_wmt_provider_db.admin.process_requests(party_account string)
returns string
language javascript
comment='{"origin":"sf_ps_wls","name":"dcr","version":{"major":5, "minor":5},"attributes":{"component":"dcr",“role”:“provider”}}'
execute as owner
...
//pull request parameters from request stream
 ...
 var request_tbl_sql =`create or replace table dcr_wmt_provider_db.admin.request_tmp as select REQUEST:REQUEST_ID as REQUEST_ID,
...
from dcr_wmt_provider_db.admin.request_stream_`+PARTY_ACCOUNT+` `;

var request_tbl_sql_exec= snowflake.execute({sqlText: request_tbl_sql});
request_tbl_sql_exec.next();

  // check data in stream
  var result = snowflake.execute({ sqlText: `select count(1) from dcr_wmt_provider_db.admin.request_tmp` });
  result.next();
 ...
 // get template available fields
  var result = snowflake.execute({ sqlText: `select lower(dimensions) from dcr_wmt_provider_db.templates.dcr_templates where party_account=(select any_value(requester_account) from dcr_wmt_provider_db.admin.request_tmp) and template_name = (select any_value(query_template) from dcr_wmt_provider_db.admin.request_tmp)` });
  result.next();
  var dimensions = result.getColumnValue(1);

// get request_id
  var result = snowflake.execute({ sqlText: `select REQUEST_ID,select_fields from dcr_wmt_provider_db.admin.request_tmp` });
  result.next();
  var request_id = result.getColumnValue(1);
  var select_field = result.getColumnValue(2);

//Validation based on Input
    var unapproved_entry = ` insert into dcr_wmt_provider_db.admin.request_log (party_account, request_id, request_ts, request, app_instance_id,query_hash,    template_name, epsilon, sensitivity, processed_ts, approved,error)
        with request as (
        ...
        ,req.request
        ,req.REQUEST_PARAMS
        from dcr_wmt_provider_db.admin.request_tmp req
          )
  ...
  //1.TimeStamp check that request is not too old or from the future
     ...
     var result = snowflake.execute({ sqlText: `select request_ts from dcr_wmt_provider_db.admin.request_tmp` });
     result.next();
 ...
 // verify list of available values
    if (select_field == null) {
      ...
      // verify list of available values
        ...
        from dcr_wmt_provider_db.admin.request_tmp)
        ,tem_dim as
...
var sql_text = `
insert into dcr_wmt_provider_db.admin.request_log (party_account, request_id, request_ts, request, app_instance_id,  query_hash, template_name, epsilon, sensitivity, processed_ts, approved,error)
with request as (
 select req.REQUEST_ID
 ,req.query_template query_template
 ,req.requester_account requester_account
...
proposed_query as (
 select dcr_wmt_provider_db.templates.get_sql_jinja(
     (select any_value(template) from dcr_wmt_provider_db.templates.dcr_templates where template_name = rp.query_template
...
validate_sql as (
  ...
  dcr_wmt_provider_db.templates.get_sql_jinja(template, request_params) as valid_sql,
  ...
  from request ,proposed_query pq, dcr_wmt_provider_db.templates.dcr_templates t
...
snowflake.execute({ sqlText: sql_text });
  ...
  // get proposed query
    var result = snowflake.execute({ sqlText: `select query_hash from dcr_wmt_provider_db.admin.request_log where REQUEST_ID ='`+request_id+`'; ` });
    result.next();
...
// process approvals manually one-time, replace SFSENORTHAMERICA_VINODSHIV with the consumer account name

call dcr_wmt_provider_db.admin.process_requests('SFSENORTHAMERICA_VINODSHIV');

////////
/// ENABLE EACH SFSENORTHAMERICA_VINODSHIV
/// Replace all instances of SFSENORTHAMERICA_VINODSHIV below with the name of the consumer account
////////

// set up a fleet of tasks to automatically process consumer requests

CREATE OR REPLACE TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_1
  SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh'
WHEN  SYSTEM$STREAM_HAS_DATA('dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV')
AS call dcr_wmt_provider_db.admin.process_requests('SFSENORTHAMERICA_VINODSHIV');

CREATE OR REPLACE TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_2 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV') AS call dcr_wmt_provider_db.admin.process_requests('SFSENORTHAMERICA_VINODSHIV');
CREATE OR REPLACE TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_3 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV') AS call dcr_wmt_provider_db.admin.process_requests('SFSENORTHAMERICA_VINODSHIV');
CREATE OR REPLACE TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_4 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV') AS call dcr_wmt_provider_db.admin.process_requests('SFSENORTHAMERICA_VINODSHIV');
CREATE OR REPLACE TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_5 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV') AS call dcr_wmt_provider_db.admin.process_requests('SFSENORTHAMERICA_VINODSHIV');
CREATE OR REPLACE TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_6 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_wmt_provider_db.admin.request_stream_SFSENORTHAMERICA_VINODSHIV') AS call dcr_wmt_provider_db.admin.process_requests('SFSENORTHAMERICA_VINODSHIV');

// enable tasks 8 seconds apart to reduce consumer wait time for request approval

ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_1 RESUME;
call system$wait(8);
ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_2 RESUME;
call system$wait(8);
ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_3 RESUME;
call system$wait(8);
ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_4 RESUME;
call system$wait(8);
ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_5 RESUME;
call system$wait(8);
ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_6 RESUME;
show tasks in  dcr_wmt_provider_db.admin;

// use these to later pause the clean room request approval tasks, if needed

//ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_1 SUSPEND;
//ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_2 SUSPEND;
//ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_3 SUSPEND;
//ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_4 SUSPEND;
//ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_5 SUSPEND;
//ALTER TASK dcr_wmt_provider_db.admin.process_requests_SFSENORTHAMERICA_VINODSHIV_6 SUSPEND;
